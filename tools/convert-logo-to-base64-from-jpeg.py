#!/usr/bin/env python3
"""
Convert logo JPEG to base64 data URL for PDFMake
Uses the user-provided white-background JPEG directly

Usage: python tools/convert-logo-to-base64-from-jpeg.py
...
def convert_jpeg_to_base64(source_path, output_js_path, target_width=80):
    """
    Convert JPEG image to base64 data URL for PDFMake
    - Uses existing JPEG directly (already has white background)
 ...
 * Source: {source_path.name} (JPEG with white background)
 * Original: {original_width}x{original_height} pixels, {original_mode} mode
 * Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
 *
 * Note: Using JPEG format with white background (no transparency issues)
 * To regenerate: python tools/convert-logo-to-base64-from-jpeg.py
 *
...
'''
        ...
        print(f"  Format:        JPEG (white background, no transparency)")
...
if __name__ == '__main__':
    ...
    source_path = project_root / 'assets' / 'images' / 'FVU-logo-white-bg.jpg'
    output_path = project_root / 'assets' / 'js' / 'logo-data.js'

    if not source_path.exists():
        print(f"ERROR: Source file not found: {source_path}")
        print("Please ensure FVU-logo-white-bg.jpg exists in assets/images/")
        sys.exit(1)

    convert_jpeg_to_base64(source_path, output_path)
